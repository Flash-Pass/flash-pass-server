...
func newPlan(db *gorm.DB, opts ...gen.DOOption) plan {
	...
	_plan.Title = field.NewString(tableName, "title")
	_plan.Description = field.NewString(tableName, "description")
	_plan.CycleSize = field.NewInt(tableName, "cycle_size")
	_plan.LearnPerCycle = field.NewInt(tableName, "learn_per_cycle")
	_plan.ReviewPerCycle = field.NewInt(tableName, "review_per_cycle")
	_plan.ReviewPerLearn = field.NewInt(tableName, "review_per_learn")
	_plan.GroupSize = field.NewInt(tableName, "group_size")
...
type plan struct {
	...
	CycleSize      field.Int
	LearnPerCycle  field.Int
	ReviewPerCycle field.Int
	ReviewPerLearn field.Int
	GroupSize      field.Int
...
func (p *plan) updateTableName(table string) *plan {
	...
	p.CycleSize = field.NewInt(table, "cycle_size")
	p.LearnPerCycle = field.NewInt(table, "learn_per_cycle")
	p.ReviewPerCycle = field.NewInt(table, "review_per_cycle")
	p.ReviewPerLearn = field.NewInt(table, "review_per_learn")
	p.GroupSize = field.NewInt(table, "group_size")
...
func (p *plan) fillFieldMap() {
	...
	p.fieldMap["cycle_size"] = p.CycleSize
	p.fieldMap["learn_per_cycle"] = p.LearnPerCycle
	p.fieldMap["review_per_cycle"] = p.ReviewPerCycle
	p.fieldMap["review_per_learn"] = p.ReviewPerLearn
	p.fieldMap["group_size"] = p.GroupSize
	p.fieldMap["review_cycles"] = p.ReviewCycles
	p.fieldMap["learn_strategy"] = p.LearnStrategy
	p.fieldMap["review_strategy"] = p.ReviewStrategy
...
func (p planDo) GetBySearchAndUserId(search string, userId uint64) (result []*model.Plan, err error) {
	...
	generateSQL.WriteString("SELECT * FROM plans WHERE question LIKE concat(\"%\", ?,\"%\") OR answer LIKE concat(\"%\", ?,\"%\") OR created_by = ? ")

...
func newTask(db *gorm.DB, opts ...gen.DOOption) task {
	...
	_task.IsDeleted = field.NewBool(tableName, "is_deleted")
	_task.PlanId = field.NewInt64(tableName, "plan_id")
	_task.BookId = field.NewInt64(tableName, "book_id")
	_task.Name = field.NewString(tableName, "name")
	_task.IsActive = field.NewBool(tableName, "is_active")
	_task.CreatedBy = field.NewInt64(tableName, "created_by")
...
func (t *task) updateTableName(table string) *task {
	...
	t.BookId = field.NewInt64(table, "book_id")
...
type ITaskDo interface {
	...
	GetBySearchAndUserId(search string, userId int64) (result []*model.Task, err error)
}

// SELECT * FROM @@table WHERE question LIKE concat("%", @search,"%") OR answer LIKE concat("%", @search,"%") OR created_by = @userId
func (t taskDo) GetBySearchAndUserId(search string, userId int64) (result []*model.Task, err error) {
	...
	generateSQL.WriteString("SELECT * FROM tasks WHERE question LIKE concat(\"%\", ?,\"%\") OR answer LIKE concat(\"%\", ?,\"%\") OR created_by = ? ")

...
func newCard(db *gorm.DB, opts ...gen.DOOption) card {
	...
	_card.Id = field.NewInt64(tableName, "id")
	...
	_card.CreatedBy = field.NewInt64(tableName, "created_by")
...
type ICardDo interface {
	...
	GetBySearchAndUserId(search string, userId int64) (result []*model.Card, err error)
}

// SELECT * FROM @@table WHERE question LIKE concat("%", @search,"%") OR answer LIKE concat("%", @search,"%") OR created_by = @userId
func (c cardDo) GetBySearchAndUserId(search string, userId int64) (result []*model.Card, err error) {
